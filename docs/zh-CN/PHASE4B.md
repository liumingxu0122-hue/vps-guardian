...
Controller 签发短期令牌，生成的命令通过 `0600` 令牌文件传递。Phase 4C 在此边界上增加 Agent 本地生成私钥，并通过私有 Agent Gateway 完成 CSR Bootstrap。证书续签和 CRL 发布仍是受控、带审计的操作。实现和验收状态参见 [Phase 4C Staging 说明](PHASE4C.md)。
